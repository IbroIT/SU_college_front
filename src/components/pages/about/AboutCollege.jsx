...
const UltimateHero = () => {
  ...
  // Динамические данные из переводов
  const stats = [
    { 
      icon: <FaBriefcase className="text-yellow-400" />, 
      value: t('hero.stats.experience.value'), 
      label: t('hero.stats.experience.label'), 
      suffix: t('hero.stats.experience.suffix') 
    },
    { 
      icon: <FaGraduationCap className="text-blue-400" />, 
      value: t('hero.stats.directions.value'), 
      label: t('hero.stats.directions.label'), 
      suffix: t('hero.stats.directions.suffix') 
    },
    { 
      icon: <FaCalendarAlt className="text-red-400" />, 
      value: t('hero.stats.year.value'), 
      label: t('hero.stats.year.label'), 
      suffix: t('hero.stats.year.suffix') 
    }
  ];
  ...
  const universityName = t('hero.universityName');
  ...
  useEffect(() => {
    ...
    const typing = setInterval(() => {
      if (i < universityName.length) {
        setDisplayedTitle(universityName.substring(0, i + 1));
        i++;
      } else {
