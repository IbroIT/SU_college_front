  ...
  return (
    <div className="min-h-screen bg-white relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-12 md:py-16 lg:py-20">
        ...
        >
          {stats.map((stat, index) => (
            <motion.div
              ...
              className="text-center p-4 md:p-6 bg-white rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-16 lg:mb-20">
          {/* Навигация по турам */}
          <motion.div
          ...
          >
            <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 p-4 md:p-6 sticky top-6">
              ...
              <div className="space-y-3">
                {selectionTours.map((tour, index) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`w-full text-left p-3 md:p-4 rounded-2xl border-2 transition-all duration-300 ${
                      activeTour === index
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.3 }}
            className="lg:hidden order-1 mb-4 md:mb-6"
          >
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {selectionTours.map((tour, index) => (
                  <motion.button
                    ...
                    onClick={() => setActiveTour(index)}
                    whileTap={{ scale: 0.95 }}
                    className={`flex-shrink-0 px-3 md:px-4 py-2 md:py-3 rounded-xl border-2 transition-all duration-300 text-xs md:text-sm font-semibold min-w-max ${
                      activeTour === index
                        ? `border-blue-500 bg-gradient-to-r ${tour.color} text-white shadow-md`
                        : 'border-gray-200 bg-gray-50 hover:bg-gray-100 text-gray-700'
                    }`}
                  >
                    <div className="flex items-center gap-1 md:gap-2">
                      <span className="whitespace-nowrap">{tour.tour}</span>
                      <div className={`px-1.5 md:px-2 py-0.5 rounded-full text-xs font-medium ${
                        activeTour === index
                          ? 'bg-white/20 text-white'
                          : getStatusColor(tour.status)
                      }`}>
          ...
          >
            <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 p-6 md:p-8">
                ...
                <table className="w-full min-w-[500px] md:min-w-[600px]">
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="px-3 md:px-4 lg:px-6 py-2 md:py-3 lg:py-4 text-left font-semibold text-gray-700 border-b border-gray-200 text-sm md:text-base">
                  ...
                  <tbody>
                    {selectionTours[activeTour].actions.map((action, index) => (
                      ...
                      >
                        ...
                        <td className="px-3 md:px-4 lg:px-6 py-2 md:py-3 lg:py-4">
                          <span className={`inline-flex items-center gap-1 px-2 md:px-3 py-1 rounded-full text-xs md:text-sm font-medium ${getStatusColor(action.status)}`}>
              ...
              <div className="mt-4 md:mt-6 lg:mt-8 p-3 md:p-4 lg:p-6 bg-blue-50 rounded-2xl border border-blue-200">
                <h4 className="text-sm md:text-base lg:text-lg font-semibold text-gray-800 mb-2 md:mb-3 flex items-center gap-2">
                  <FaExclamationTriangle className="text-blue-500 flex-shrink-0" />
                  <span>{t('selectionSchedule.important.title')}</span>
                </h4>
                <ul className="space-y-1 md:space-y-2">
                  {importantNotes.map((note, index) => (
                    <li key={index} className="flex items-start gap-2 text-gray-700">
                      <FaCheckCircle className="text-blue-500 mt-0.5 md:mt-1 flex-shrink-0" size={12} />
                      <span className="text-xs md:text-sm leading-relaxed">{note}</span>
                    </li>
                  ))}
                </ul>
              </div>
